 ...
 *            中文说明：Newlib/Picolibc 的底层系统调用桩。裸机无文件系统和
 *            进程，多数桩直接返回错误；_write/_read 把字符转发给弱符号
 *            __io_putchar/__io_getchar，但本工程未实现这两个弱符号，
 *            printf 族没有可用输出后端——控制台打印请用 rt_kprintf()
 *            （最终走 BSP/rt_hw_console.c -> USART1）。
 ******************************************************************************
 * @attention
 *
...
/* _read/_write 是 newlib 中 scanf/printf 的最终出口，逐字符转发给
 * __io_getchar/__io_putchar（弱符号，本工程未实现，见文件头说明）。 */
...
/* 以下桩对应裸机不存在的能力（文件系统、进程等），按惯例返回错误，
 * 需要区分原因时置 errno。 */
int _close(int file)
{
  (void)file;
